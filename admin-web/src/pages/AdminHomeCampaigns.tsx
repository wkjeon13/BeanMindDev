...
const AdminHomeCampaigns = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      ...
      {/* FLASH DROP / EVENT BANNER */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
        <div className="p-6 border-b border-gray-200 flex justify-between items-center bg-gray-50">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Event Banner (Flash Drop)</h2>
            <p className="text-sm text-gray-500 mt-1">Displayed directly below the Hero section.</p>
          </div>
          <div className="flex items-center gap-4">
            <button 
              onClick={() => handleSaveSection('HOME_FLASH_DROP')}
              disabled={saving['HOME_FLASH_DROP']}
            ...
            >
              ...
              {saving['HOME_FLASH_DROP'] ? 'Saving...' : 'Save Section'}
            </button>
            <label className="flex items-center cursor-pointer">
              <div className="relative">
                <input type="checkbox" className="sr-only" checked={config.HOME_FLASH_DROP.isActive} onChange={(e) => handleChange('HOME_FLASH_DROP', 'isActive', e.target.checked)} />
                <div className={`block w-14 h-8 rounded-full ${config.HOME_FLASH_DROP.isActive ? 'bg-indigo-500' : 'bg-gray-300'}`}></div>
                <div className={`dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition transform ${config.HOME_FLASH_DROP.isActive ? 'translate-x-6' : ''}`}></div>
              </div>
              <span className="ml-3 font-medium text-gray-900">{config.HOME_FLASH_DROP.isActive ? 'Active' : 'Inactive'}</span>
            </label>
          </div>
        </div>
        
        {config.HOME_FLASH_DROP.isActive && (
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Badge Text (e.g., Flash Drop : 게릴라 특가)</label>
              <input type="text" className="w-full p-2 border border-gray-300 rounded-lg" value={config.HOME_FLASH_DROP.badgeText || ''} onChange={(e) => handleChange('HOME_FLASH_DROP', 'badgeText', e.target.value)} />
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Badge Text (English)</label>
              <input type="text" className="w-full p-2 border border-gray-300 rounded-lg" value={config.HOME_FLASH_DROP.badgeTextEn || ''} onChange={(e) => handleChange('HOME_FLASH_DROP', 'badgeTextEn', e.target.value)} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Title</label>
              <input type="text" className="w-full p-2 border border-gray-300 rounded-lg" value={config.HOME_FLASH_DROP.title || ''} onChange={(e) => handleChange('HOME_FLASH_DROP', 'title', e.target.value)} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Title (English)</label>
              <input type="text" className="w-full p-2 border border-gray-300 rounded-lg" value={config.HOME_FLASH_DROP.titleEn || ''} onChange={(e) => handleChange('HOME_FLASH_DROP', 'titleEn', e.target.value)} />
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
              <textarea className="w-full p-2 border border-gray-300 rounded-lg" rows={2} value={config.HOME_FLASH_DROP.description || ''} onChange={(e) => handleChange('HOME_FLASH_DROP', 'description', e.target.value)}></textarea>
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Description (English)</label>
              <textarea className="w-full p-2 border border-gray-300 rounded-lg" rows={2} value={config.HOME_FLASH_DROP.descriptionEn || ''} onChange={(e) => handleChange('HOME_FLASH_DROP', 'descriptionEn', e.target.value)}></textarea>
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Target Link URL</label>
              <input type="text" className="w-full p-2 border border-gray-300 rounded-lg" value={config.HOME_FLASH_DROP.linkUrl || ''} onChange={(e) => handleChange('HOME_FLASH_DROP', 'linkUrl', e.target.value)} />
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-2 flex justify-between">
                <span>Background Image URL</span>
                <label className="cursor-pointer text-indigo-600 hover:text-indigo-800 flex items-center text-xs">
                  {uploading['HOME_FLASH_DROP'] ? <Loader2 size={14} className="animate-spin mr-1"/> : <ImageIcon size={14} className="mr-1"/>}
                  Upload Image
                  <input type="file" className="hidden" accept="image/*" onChange={(e) => handleFileUpload(e, 'HOME_FLASH_DROP', 'imageUrl')} disabled={uploading['HOME_FLASH_DROP']} />
                </label>
              </label>
              <input type="text" className="w-full p-2 border border-gray-300 rounded-lg" value={config.HOME_FLASH_DROP.imageUrl || ''} onChange={(e) => handleChange('HOME_FLASH_DROP', 'imageUrl', e.target.value)} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Start Time</label>
              <input type="datetime-local" className="w-full p-2 border border-gray-300 rounded-lg" 
                value={config.HOME_FLASH_DROP.startTime ? config.HOME_FLASH_DROP.startTime.substring(0,16) : ''} 
                onChange={(e) => handleChange('HOME_FLASH_DROP', 'startTime', new Date(e.target.value).toISOString())} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">End Time</label>
              <input type="datetime-local" className="w-full p-2 border border-gray-300 rounded-lg" 
                value={config.HOME_FLASH_DROP.endTime ? config.HOME_FLASH_DROP.endTime.substring(0,16) : ''} 
                onChange={(e) => handleChange('HOME_FLASH_DROP', 'endTime', new Date(e.target.value).toISOString())} />
            </div>
          </div>
        )}
      </div>
