        ...
        return (
            <section className="pt-6 pb-2 border-t border-espresso-800 bg-gradient-to-b from-espresso-950 to-[#160d08]">
                <div className="px-4 flex items-center justify-between mb-4">
                    <div>
                        <h3 className="text-[17px] font-bold flex items-center gap-2 text-espresso-50">
                            ...
                            {t('home.title_pairing', '?§Îäò???ÑÎ≤Ω???òÏñ¥Îß?)}
                        </h3>
                    </div>
                </div>
                <div className="px-4 py-8 text-center text-espresso-400 text-[13px] bg-espresso-900/50 mx-4 rounded-2xl border border-espresso-800 border-dashed">
                    ?ÑÏû¨ ÏßÄ??óê Ï∂îÏ≤ú???òÏñ¥Îß??∞Ïù¥?∞Í? ?ÜÏäµ?àÎã§.<br/>
                    (?àÎ°úÍ≥†Ïπ®???òÍ±∞??Í¥ÄÎ¶¨Ïûê ?òÏù¥ÏßÄÎ•??ïÏù∏?¥Ï£º?∏Ïöî)
                </div>
            </section>
        );
    }
        ...
        <section className="pt-6 pb-2 border-t border-espresso-800 bg-gradient-to-b from-espresso-950 to-[#160d08]">
            <div className="px-4 flex items-center justify-between mb-4">
                <div>
                    ...
                    <p className="text-[12px] text-espresso-400 mt-1">{t('home.desc_pairing', 'ÏßÄÍ∏?Î®πÍ≥† ?∂Ï? ?îÏ??∏Î? Í≥®ÎùºÎ≥¥ÏÑ∏??')}</p>
                </div>
            </div>
            ...
            <div className="flex gap-2 overflow-x-auto px-4 pb-4 snap-x hide-scrollbar">
                {todayPairings.map((item, idx) => {
                    ...
                    const itemName = (item && item.name) ? item.name : '?îÏ???;
                    return (
                        <button 
                            key={idx}
                        ...
                        >
                            {isImageUrl ? (
                                ...
                                <>
                                    <span className="text-[32px] mb-1 drop-shadow-sm">{iconStr || '?ç∞'}</span>
                ...
                {activeItem && (
                    ...
                    >
                        <div className="bg-gradient-to-br from-[#2a1a10] to-[#1a100a] border border-amber-500/30 rounded-2xl p-4 shadow-lg flex items-center gap-4">
                            ...
                            <div>
                                <div className="text-[11px] font-bold text-amber-500 mb-1">{t('home.pairing_recommendation', { name: activeItem.name || '?îÏ??? })}</div>
                                <div className="text-[15px] font-black text-espresso-50 mb-1">{activeItem.coffee || 'Ï∂îÏ≤ú Ïª§Ìîº'}</div>
                                <div className="text-[12px] text-espresso-300 line-clamp-2 leading-snug">{activeItem.desc || '?§Î™Ö???ÜÏäµ?àÎã§.'}</div>
                            </div>
                        </div>
                    </motion.div>
                )}
            ...
            <div className="px-4 flex items-center justify-between mb-3 mt-2">
                <h4 className="text-[14px] font-bold text-espresso-200">{t('home.title_user_pairing', '?†Ï??§Ïùò ?òÏñ¥Îß?Ï∂îÏ≤ú')}</h4>
            </div>
                    ...
                    <div key={post.id} onClick={() => navigate('/community', { state: { activePost: post.id } })} className="relative w-[calc(33.333%-8px)] aspect-[3/4] max-h-[180px] rounded-2xl overflow-hidden shrink-0 snap-center shadow-lg group cursor-pointer border border-espresso-800">
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-[#120a05]/90 via-[#120a05]/30 to-transparent flex flex-col justify-end p-3 pointer-events-none">
                            ...
                            <div className="flex justify-between items-center w-full">
                                <span className="text-[10px] text-espresso-300">{post.author?.nickname || 'Ïª§Ìîº?¨Î≤Ñ'}</span>
...
export default function HomeDashboard() {
  ...
  const handleSaveLayout = async (newLayout: HomeSectionConfig[]) => {
    if (isLoggedIn) {
      try {
        ...
        if (!res.ok) {
           alert("?§Ï†ï ?Ä?•Ïóê ?§Ìå®?àÏäµ?àÎã§. ?§Ïãú Î°úÍ∑∏?∏Ìï¥Ï£ºÏÑ∏??");
           return; // Abort if save failed!
        }
      } catch (error) {
        console.error("Failed to save layout", error);
        alert("?§Ï†ï ?Ä?•Ïóê ?§Ìå®?àÏäµ?àÎã§. ?§Ìä∏?åÌÅ¨Î•??ïÏù∏?¥Ï£º?∏Ïöî.");
        return;
      }
    }
  ...
  const greetingName = currentUser?.nickname || t('home.guest', 'Î∞©Î¨∏??);
  ...
  if (config.id === 'hero') {
                      ...
                      {personalizedData?.latestPrescription ? (
                          <>
                              ...
                              <span className="block text-[22px] font-serif leading-[1.1] tracking-tight text-white/95">
                                {t('home.lbl_previous_suggestion', 'ÏßÄ?úÎ≤à Ï∂îÏ≤úÎ∞õÏúº??)}
                              </span>
                              ...
                              <span className="block text-[11px] font-light text-espresso-200 mt-2 leading-relaxed max-w-[80%]">{t('home.lbl_ask_experience', '?¥Îñ†?®ÎÇò?? ?πÏã†???ÑÌïú ?àÎ°ú?????îÏùÑ ?úÏïà?©Îãà??')}</span>
                          </>
                      ) : (
                          <>
                              <span className="block text-[12px] text-espresso-200 font-light mb-2 tracking-widest uppercase">
              ...
              <section key={config.id} className="w-full md:w-1/2 lg:w-1/3 md:px-2 py-2 mt-1">
                 <div className="px-6 flex items-center justify-between mb-3">
                   <h3 className="text-[20px] font-serif tracking-tight text-white flex items-center gap-2">
                     <Users className="text-blue-500 w-4 h-4" /> {t('home.title_following_news', '???¥ÏõÉ & ?®Í≥® ?åÏãù')}
                   </h3>
                 </div>
      ...
      if (config.id === 'hot_feeds') return personalizedData && (
          <section key={config.id} className="w-full md:w-1/2 lg:w-1/3 md:px-2 py-2 mt-1">
             <div className="px-6 flex items-center justify-between mb-3">
               <h3 className="text-[20px] font-serif tracking-tight text-white flex items-center gap-2">
                 <Flame className="text-amber-500 w-4 h-4" /> {t('home.title_hot_feeds', '?∏Í∏∞ Ïª§Ìîº??)}
               </h3>
               <button onClick={() => navigate('/community', { state: { filter: 'hot_3m' } })} className="text-[12px] text-espresso-400 font-medium">{t('home.btn_more', '?îÎ≥¥Í∏?)}</button>
             </div>
             <div className="flex gap-3 overflow-x-auto px-4 pb-6 snap-x hide-scrollbar">
                {(!personalizedData.hotCoffeeTalkFeeds || personalizedData.hotCoffeeTalkFeeds.length === 0) ? (
                    <div className="w-full flex flex-col items-center justify-center py-8 text-[13px] text-espresso-400 bg-espresso-900/20 rounded-2xl border border-espresso-800/50">
                        ...
                        ÏµúÍ∑º 1?¨Í∞Ñ ?∏Í∏∞?àÎäî ?ºÎìúÍ∞Ä ?ÜÏäµ?àÎã§.
                    </div>
          ...
          <section key={config.id} className="w-full md:w-1/2 lg:w-1/3 md:px-2 py-2 mb-2">
             <div className="px-6 flex items-center justify-between mb-3">
               <h3 className="text-[18px] font-serif tracking-tight text-white flex items-center gap-2">
                 <Zap className="text-blue-400 w-4 h-4" /> {t('home.title_new_feeds', 'ÏµúÏã† ?ºÎìú')}
               </h3>
             </div>
      ...
      if (config.id === 'taste_match') return isLoggedIn && personalizedData && personalizedData.tasteMatchedFeeds && personalizedData.tasteMatchedFeeds.length > 0 && (
              <section key={config.id} className="w-full md:w-1/2 lg:w-1/3 md:px-2 py-2 mt-1">
                 <div className="px-6 flex items-center justify-between mb-3">
                   <h3 className="text-[20px] font-serif tracking-tight text-white flex items-center gap-2">
                     <Sparkles className="text-amber-500 w-4 h-4" /> {t('home.title_taste_match', '{{name}}??Ï∑®Ìñ• ?ÄÍ≤??ºÎìú', { name: greetingName })}
                   </h3>
                 </div>
                 <div className="flex gap-3 overflow-x-auto px-4 pb-6 snap-x hide-scrollbar">
      ...
      if (config.id === 'following') return isLoggedIn && personalizedData && (
          <section key={config.id} className="w-full md:w-1/2 lg:w-1/3 md:px-2 py-2 mt-1">
             <div className="px-6 flex items-center justify-between mb-3">
               <h3 className="text-[20px] font-serif tracking-tight text-white flex items-center gap-2">
                 <Heart className="text-pink-500 w-4 h-4" /> {t('home.title_following', '?îÎ°ú???åÏãù')}
               </h3>
               <button onClick={() => navigate('/community', { state: { filter: 'following_story' } })} className="text-[12px] text-espresso-400 font-medium">{t('home.btn_more', '?îÎ≥¥Í∏?)}</button>
             </div>
             <div className="flex gap-3 overflow-x-auto px-4 pb-6 snap-x hide-scrollbar">
                {(!personalizedData.followingFeeds || personalizedData.followingFeeds.length === 0) ? (
                    <div className="w-full flex flex-col items-center justify-center py-8 text-[13px] text-espresso-400 bg-espresso-900/20 rounded-2xl border border-espresso-800/50">
                        <Heart size={24} className="mb-2 opacity-50" />
                        ?îÎ°ú?∞Ìïú ?¨Ïö©?êÏùò ?åÏãù???ÜÏäµ?àÎã§.
                    </div>
                ) : personalizedData.followingFeeds.map((post: any) => (
                  <div 
                    key={post.id}
            ...
            if (config.id === 'my_clubs') return isLoggedIn && personalizedData && personalizedData.myClubFeeds && personalizedData.myClubFeeds.length > 0 && (
              <section key={config.id} className="w-full md:w-1/2 lg:w-1/3 md:px-2 py-2 mt-1">
                 <div className="px-6 flex items-center justify-between mb-3">
                   ...
                   <button onClick={() => navigate('/clubs')} className="text-[12px] text-espresso-400 font-medium">{t('home.btn_more', '?îÎ≥¥Í∏?)}</button>
                 </div>
        ...
        <section key={config.id} className="w-full md:w-1/2 lg:w-1/3 md:px-2 py-2">
          <div className="px-4 flex items-center justify-between mb-4 mt-2">
            <h3 className="text-[22px] font-serif tracking-tight text-white flex items-center gap-2">
              <Video className="text-amber-500 w-5 h-5" /> {t('home.title_shorts', '1Î∂?Ïª§Ìîº ?êÌóò')}
            </h3>
          </div>
          <div className="grid grid-cols-2 gap-3 px-4 pb-4">
            {isLoading ? (
              ...
              <div className="col-span-2 aspect-[4/5] w-full flex items-center justify-center text-espresso-400 text-[13px] font-medium bg-espresso-900/20 rounded-2xl border border-espresso-800/50">
                  ?ÑÏßÅ ?∏Í∏∞?àÎäî ?èÌèº ?ºÎìúÍ∞Ä ?ÜÏäµ?àÎã§.
              </div>
        ...
        <section key={config.id} className="w-full md:w-1/2 lg:w-1/3 md:px-2 py-2 mt-1">
          <div className="px-6 flex items-center justify-between mb-3">
            <h3 className="text-[20px] font-serif tracking-tight text-white flex items-center gap-2">
              <MapPin className="text-amber-500 w-4 h-4" /> {t('home.title_trending_cafes', '?îÏ¶ò ?®Îäî ?±Ï?')}
            </h3>
            <button onClick={() => navigate('/community', { state: { filter: 'pilgrimage_talk' } })} className="text-[12px] text-espresso-400 font-medium">{t('home.btn_more', '?îÎ≥¥Í∏?)}</button>
          </div>
          <div className="flex gap-3 overflow-x-auto px-4 pb-4 snap-x hide-scrollbar">
            {isLoading ? (
              [1, 2, 3].map(i => (
              ...
              <div className="w-full aspect-square flex flex-col items-center justify-center text-espresso-400 bg-espresso-900/20 rounded-2xl border border-espresso-800/50">
                  ...
                  <span className="text-[13px] font-medium">??ÏßÄ??ùò ?®Îäî ?±Ï?Í∞Ä ?ÜÏäµ?àÎã§.</span>
              </div>
        ...
        <section key={config.id} className="w-full md:w-1/2 lg:w-1/3 md:px-2 pt-2 pb-4">
          <div className="px-4 flex items-center justify-between mb-3">
            <h3 className="text-[16px] font-bold flex items-center gap-1.5">
              <Users className="text-amber-500 w-4 h-4" /> {t('home.title_recommended_clubs', '?∞Î¶¨ ?ôÎÑ§ Ï∂îÏ≤ú ?¨Î£®')}
            </h3>
            <button onClick={() => navigate('/clubs')} className="text-[12px] text-espresso-400 font-medium">{t('home.btn_more', '?îÎ≥¥Í∏?)}</button>
          </div>
          <div className="grid grid-cols-2 gap-3 px-4 pb-4">
            {isLoading ? (
              ...
              <div className="col-span-2 w-full h-[120px] flex flex-col items-center justify-center text-espresso-400 bg-espresso-900/20 rounded-2xl border border-espresso-800/50">
                  ...
                  <span className="text-[13px] font-medium text-center">Î∞òÍ≤Ω 20km ?¥Ïóê ?úÎ∞ú???¨Î£®Í∞Ä ?ÜÏäµ?àÎã§.<br/><span className="text-[11px]">ÏßÅÏ†ë Î™®ÏûÑ???¥Ïñ¥Î≥¥Ïãú??Í±??¥Îñ®ÍπåÏöî?</span></span>
              </div>
                ...
                <div className="w-full h-[90px] bg-espresso-800 overflow-hidden relative">
                  ...
                  {club.isRecruiting && (
                    <div className="absolute top-2 left-2 bg-amber-500 text-espresso-950 text-[10px] font-bold px-1.5 py-0.5 rounded-sm shadow-sm">
                      {t('home.badge_recruiting', 'Î™®ÏßëÏ§?)}
                    </div>
                  )}
                </div>
                <div className="p-3">
                  ...
                  <div className="flex items-center justify-between text-[10px] font-medium text-espresso-300">
                    <span className="flex items-center gap-1 truncate max-w-[80px]">
                      <MapPin size={10} className="shrink-0" /> {club.locationName || t('home.lbl_nationwide', '?ÑÍµ≠')}
                    </span>
          ...
          >
            ...
            >
              ...
              <h3 className="text-xl font-bold text-espresso-50 text-center mb-3">
                {t('curator.resume_popup_title', 'AI Î∂ÑÏÑù ÏßÑÌñâ Ï§?)}
              </h3>
              
              <p className="text-sm text-espresso-200 text-center mb-8 leading-relaxed break-keep">
                {t('curator.resume_popup_desc', '?ÑÏû¨ AI Ï∑®Ìñ• Ï∂îÏ≤ú Î∂ÑÏÑù??ÏßÑÌñâ Ï§ëÏûÖ?àÎã§. ?¥Ïñ¥??ÏßÑÌñâ?òÏãúÍ≤†Ïäµ?àÍπå, ?ÑÎãàÎ©??òÏ§ë???§Ïãú ?ïÏù∏?òÏãúÍ≤†Ïäµ?àÍπå?')}
              </p>
              ...
              <div className="flex flex-col gap-3">
                ...
                >
                  {t('curator.resume_popup_continue', 'AI Ï∂îÏ≤ú Î∞õÍ∏∞ ÏßÑÌñâ ?ÅÌô©Î≥¥Í∏∞')}
                </button>
                <button 
                  onClick={() => {
                ...
                >
                  {t('curator.resume_popup_later', '?òÏ§ë???ïÏù∏?òÍ∏∞')}
                </button>
              </div>
            </motion.div>
          </motion.div>
