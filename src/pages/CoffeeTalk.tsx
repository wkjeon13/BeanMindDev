...
export default function CoffeeTalk() {
    ...
    const filteredPosts = posts.filter(post => {
        ...
        if (activeFilter === 'home_cafe') {
            const tags = [t('coffee_talk.tag_gear_boast', '장비자랑'), t('coffee_talk.tag_home_brewing', '홈브루잉'), t('coffee_talk.tag_home_cafe_life', '홈카페일상')]; return tags.some(tag => post.content?.includes(`#${tag}`));
        }
                                        ...
                                        {BGM_THEMES.map(theme => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`shrink-0 snap-center px-3 py-2 rounded-xl text-xs font-bold transition-all border flex items-center gap-1.5 active:scale-95 ${isSelected
