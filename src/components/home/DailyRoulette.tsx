...
const DailyRoulette = () => {
    ...
    return (
        <section className="px-4 py-2 bg-espresso-950">
            ...
            <div className="mb-4 bg-[#111111] rounded-xl p-4 border-2 border-espresso-700 shadow-[4px_4px_0px_#2a1a10]">
                <div className="flex justify-between items-center mb-3">
                    <h3 className="text-white font-black text-[14px] flex items-center gap-1.5">
                        <Flame className="text-amber-500 fill-amber-500/20" size={16}/> 7일 출석 챌린지
                    </h3>
                    <span className="text-amber-500 text-[12px] font-black border-2 border-amber-500 px-2 py-0.5 bg-[#222]">
                        {status.streak} / 7일
                    </span>
                </div>
                <div className="flex justify-between gap-1">
                    {[1,2,3,4,5,6,7].map(day => (
                        <div key={day} className="flex flex-col items-center gap-1.5 flex-1 relative group cursor-default">
                            <div className={`w-full h-2 transition-all duration-500 ${day <= status.streak ? 'bg-amber-500 shadow-[2px_2px_0px_#f59e0b]' : 'bg-espresso-800'}`} />
                            <span className={`text-[10px] font-black ${day <= status.streak ? 'text-amber-500' : 'text-espresso-500'}`}>
                            ...
                            {/* Tooltip for Day 7 */}
                            {day === 7 && (
                                <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-amber-500 text-black text-[10px] font-black px-2 py-1 border-2 border-black shadow-[2px_2px_0px_#000] whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                    500 콩 잭팟!
                                </div>
                            )}
                        </div>
            ...
            {/* 3-Cup Game Area */}
            <div className="bg-[#111111] rounded-xl p-6 border-2 border-amber-500 text-center relative overflow-hidden shadow-[4px_4px_0px_#f59e0b]">

                {!status.todayPlayed || (status.todayPlayed && selectedCup !== null && isShuffling) ? (
                    <>
                        <h4 className="text-amber-500 font-black text-[16px] mb-10 tracking-tight flex items-center justify-center gap-2">
                        ...
                        <div className="flex justify-center gap-6">
                            {[0, 1, 2].map((i) => (
                                ...
                                >
                                    <div className={`w-[60px] h-[80px] border-2 flex items-center justify-center relative shadow-[4px_4px_0px_#000] transition-all ${selectedCup === i ? 'bg-amber-500 border-amber-500' : 'bg-[#222] border-espresso-600'}`}>
                                        <div className="absolute top-0 w-full h-3 border-b-2 border-espresso-950/30" />
                                        <Coffee className={`${selectedCup === i ? 'text-black' : 'text-espresso-500'} z-10 mt-2`} size={24} />
                                    </div>
                                    
                                    {selectedCup === i && <span className="absolute -top-6 text-[13px] font-black text-amber-500 bg-[#111] border-2 border-amber-500 px-1 py-0.5 shadow-[2px_2px_0px_#f59e0b] z-20">PICK!</span>}
                                </motion.div>
                            ))}
                        </div>
                    </>
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center">
                             ...
                             {[0, 1, 2].map((i) => (
                                <div key={i} className={`w-[70px] flex flex-col items-center justify-end relative ${selectedCup === i ? 'scale-110 z-10' : 'opacity-50 scale-90'}`}>
                                    ...
                                    <motion.div 
                                        ...
                                        className={`w-[60px] h-[60px] flex flex-col items-center justify-center mb-3 shadow-[4px_4px_0px_#000] border-2 ${selectedCup === i ? 'bg-amber-500 border-amber-500 text-black' : 'bg-[#222] border-espresso-600 text-espresso-400'}`}
                                    >
                                        <span className={`font-black ${selectedCup === i ? 'text-[22px]' : 'text-[16px]'}`}>
                                            {rewards ? rewards[i] : '?'}
                                        </span>
                                        <span className="text-[9px] font-black opacity-80 -mt-1">BEANS</span>
                                    </motion.div>
                                    <span className={`text-[11px] font-black border-2 px-2 py-0.5 ${selectedCup === i ? 'bg-amber-500 border-amber-500 text-black' : 'bg-transparent border-espresso-600 text-espresso-500'}`}>
                        ...
                        <div className="bg-[#111] px-6 py-3 border-2 border-amber-500 w-full shadow-[4px_4px_0px_#f59e0b]">
                            <h4 className="text-[17px] font-black text-amber-500 mb-1">
                                {rewards && selectedCup !== null ? `축하합니다! +${rewards[selectedCup]} 콩` : '내일 다시 도전하세요!'}
                            </h4>
                            <p className="text-[12px] text-white font-bold">{message || '오늘의 챌린지를 완료했습니다.'}</p>
                        </div>
                    </motion.div>
                )}
            </div>
