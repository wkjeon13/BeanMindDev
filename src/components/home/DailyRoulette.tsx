        ...
        <section className="w-full py-4">
            ...
            <div className="px-6 mb-8">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[9px] font-bold tracking-[0.3em] text-amber-500 uppercase">Challenge</span>
                </div>
                <div className="flex justify-between items-end mb-4">
                    <h3 className="text-[20px] font-serif tracking-tight text-white">7일 출석</h3>
                    <span className="text-[12px] font-serif text-white/50">{status.streak} / 7 Days</span>
                </div>
                <div className="flex justify-between gap-1">
                    {[1,2,3,4,5,6,7].map(day => (
                        <div key={day} className="flex flex-col items-center gap-1.5 flex-1 relative group cursor-default">
                            ...
                            <span className={`text-[10px] font-serif ${day <= status.streak ? 'text-amber-500' : 'text-white/30'}`}>
                                {day === 7 ? <Star size={10} className={`inline ${day <= status.streak ? 'fill-amber-500 text-amber-500' : 'fill-transparent text-white/30'}`} /> : day}
                            </span>
                            {day === 7 && (
                                <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-white text-black text-[9px] font-bold tracking-widest uppercase px-2 py-0.5 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                    500 Beans
                                </div>
                            )}
                        </div>
            ...
            <div className="px-6 text-center relative">
                {!status.todayPlayed || (status.todayPlayed && selectedCup !== null && isShuffling) ? (
                    <>
                        <h4 className="text-[16px] font-serif text-white mb-8 tracking-wide font-light">
                            행운의 커피 컵을 골라보세요.
                        </h4>
                        <div className="flex justify-center gap-6">
                            {[0, 1, 2].map((i) => (
                                <motion.div 
                                    ...
                                    className={`w-[60px] h-[85px] cursor-pointer relative flex flex-col items-center justify-end ${selectedCup === i ? 'scale-110 z-10' : 'opacity-80 hover:opacity-100 transition-opacity'}`}
                                >
                                    <div className={`w-full h-full border flex items-center justify-center relative transition-all duration-500 ${selectedCup === i ? 'bg-amber-500 border-amber-400' : 'bg-transparent border-white/20 hover:border-white/50 backdrop-blur-md'}`}>
                                        <div className="absolute top-0 w-full h-2 border-b border-white/10" />
                                        <Coffee className={`${selectedCup === i ? 'text-black' : 'text-white/60'} z-10 mt-1`} size={24} strokeWidth={1} />
                                    </div>
                                    
                                    {selectedCup === i && <span className="absolute -top-5 text-[9px] tracking-[0.2em] font-bold uppercase text-black bg-white px-2 py-0.5 z-20">Pick</span>}
                                </motion.div>
                            ))}
                        </div>
                    </>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="flex flex-col items-center">
                        <div className="flex justify-center gap-6 mb-8 mt-2">
                             {[0, 1, 2].map((i) => (
                                <div key={i} className={`w-[60px] flex flex-col items-center justify-end relative ${selectedCup === i ? 'scale-110 z-10' : 'opacity-40 scale-90'}`}>
                                    {selectedCup === i && (
                                        <motion.div 
                                        ...
                                        >
                                            <Sparkles size={20} className="fill-amber-500" strokeWidth={1} />
                                        </motion.div>
                                    )}
                                    ...
                                    <motion.div 
                                        ...
                                        className={`w-[50px] h-[50px] rounded-full flex flex-col items-center justify-center mb-3 border ${selectedCup === i ? 'bg-amber-500 border-amber-400 text-black' : 'bg-transparent border-white/20 text-white/40 backdrop-blur-md'}`}
                                    >
                                        <span className={`font-serif ${selectedCup === i ? 'text-[20px]' : 'text-[16px]'}`}>
                                            {rewards ? rewards[i] : '?'}
                                        </span>
                                    </motion.div>
                                    <span className={`text-[9px] font-bold tracking-widest uppercase ${selectedCup === i ? 'text-amber-500' : 'text-white/40'}`}>
                        ...
                        <div className="w-full pt-6 border-t border-white/10">
                            <h4 className="text-[18px] font-serif text-white mb-1">
                                {rewards && selectedCup !== null ? `축하합니다. +${rewards[selectedCup]} Beans` : '내일 다시 도전하세요.'}
                            </h4>
                            <p className="text-[12px] text-white/50 font-light tracking-wide">{message || '오늘의 챌린지를 완료했습니다.'}</p>
                        </div>
                    </motion.div>
                )}
            </div>
