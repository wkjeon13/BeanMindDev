  ...
  const handleMatch = async () => {
    ...
    try {
      ...
      if (data.candidates && data.candidates[0].content.parts[0].text) {
          ...
          // Telemetry and auto-save logic for mobile app to sync with backend metrics
          const trackUsage = async () => {
            try {
              const token = await getItem('token');
              if (token) {
                // Track AI usage for logged-in user
                fetch(`${API_BASE}/api/users/ai-usage`, {
                  method: 'POST',
                  headers: { 'Authorization': `Bearer ${token}` }
                }).catch(err => console.warn("Mobile telemetry error:", err));

                // Auto-save prescription to profile
                fetch(`${API_BASE}/api/users/prescriptions`, {
                  ...
                  body: JSON.stringify({
                    ...
                    beanName: parsed.bestMatch.bean.name,
                    brand: parsed.bestMatch.brand.name,
                    aiComment: parsed.aiExplanation,
                    usePoints: false
                  })
                }).catch(err => console.warn("Mobile auto-save error:", err));
              } else {
                // Track AI usage for anonymous visitors
                let visitorId = await getItem('visitor_id');
                if (!visitorId) {
                  visitorId = 'anon_' + Math.random().toString(36).substring(2, 15) + '_' + Date.now();
                  await setItem('visitor_id', visitorId);
                }
                fetch(`${API_BASE}/api/analytics/ai-usage`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ visitorId })
                }).catch(err => console.warn("Mobile anon telemetry error:", err));
              }
            } catch (err) {
              console.warn("Mobile telemetry track exception:", err);
            }
          };
      ...
      <ScrollView className="flex-1 px-6 pb-20" showsVerticalScrollIndicator={false}>
            ...
            <View>
              ...
              <View className="flex-row items-center justify-between bg-espresso-900/40 border border-espresso-700/50 rounded-3xl p-3">
                {['Morning', 'Afternoon', 'Evening', 'Night'].map((time) => {
                  const iconMap: any = { Morning: Sunrise, Afternoon: Sun, Evening: Sunset, Night: Moon };
                  ...
                  return (
                    <TouchableOpacity key={time} onPress={() => setPrefs({...prefs, timeOfDay: time})} className="flex-1 items-center gap-2 py-2">
                      <View className={`w-14 h-14 rounded-full items-center justify-center ${isSel ? 'bg-amber-500/20 border border-amber-500' : 'bg-transparent'}`}>
                        <Icon size={24} color={isSel ? "#f59e0b" : "#d3cec7"}/>
                      </View>
            ...
            <View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} className="bg-espresso-900/40 border border-espresso-700/50 rounded-3xl p-3">
                {['Sunny', 'Rainy', 'Cloudy', 'Snowy', 'Hot', 'Cold'].map((weather) => {
                  const iconMap: any = { Sunny: Sun, Rainy: CloudRain, Cloudy: Cloud, Snowy: Snowflake, Hot: ThermometerSun, Cold: ThermometerSnowflake };
            ...
            <View>
              ...
              <View className="flex-row items-center justify-between bg-espresso-900/40 border border-espresso-700/50 rounded-3xl p-3">
                {['Tired', 'Focused', 'Relaxed', 'Refreshing'].map((cond) => {
                  const iconMap: any = { Tired: BatteryWarning, Focused: Brain, Relaxed: Flower2, Refreshing: Sparkles };
            ...
            <View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} className="bg-espresso-900/40 border border-espresso-700/50 rounded-3xl p-3">
                {['None', 'CaffeineSensitive', 'StomachSensitive', 'Diabetes', 'HighCholesterol'].map((h) => {
                  ...
                  const iconMap: any = { None: CheckCircle2, CaffeineSensitive: HeartPulse, StomachSensitive: Leaf, Diabetes: Activity, HighCholesterol: Stethoscope };
          ...
          <View className="space-y-8 mb-20 gap-8">
            ...
            <View>
              ...
              <View className="flex-row justify-center gap-6 bg-espresso-900/40 border border-espresso-700/50 rounded-3xl p-5">
                {[ {v:'Espresso', l:'s_espresso', i: Coffee}, 
                   {v:'Drip', l:'s_drip', i: Wind} ].map((item) => {
                  ...
                  return (
                    <TouchableOpacity key={item.v} onPress={() => setPrefs({...prefs, base: item.v})} className="items-center gap-3 px-4">
                      <View className={`w-16 h-16 rounded-full items-center justify-center ${isSel ? 'bg-amber-500/20 border border-amber-500' : 'bg-transparent'}`}>
                        <Icon size={32} color={isSel ? "#f59e0b" : "#d3cec7"}/>
                      </View>
            ...
            <View>
              ...
              <View className="flex-row justify-center gap-6 bg-espresso-900/40 border border-espresso-700/50 rounded-3xl p-5">
                {[ {v:'Regular', l:'caff_reg', i: Coffee}, 
                   {v:'Decaf', l:'caff_decaf', i: Moon} ].map((item) => {
          ...
          <View className="space-y-8 mb-20 gap-8">
            ...
            <View>
              ...
              <View className="flex-row flex-wrap justify-between gap-y-4 bg-espresso-900/40 border border-espresso-700/50 rounded-3xl p-4">
                {[
                  {v:'Chocolate', l:'f_chocolate', i: Coffee}, {v:'Nutty', l:'f_nutty', i: Coffee}, 
                  {v:'Caramel', l:'f_caramel', i: Droplet}, {v:'Floral', l:'f_floral', i: Flower2}, 
                  {v:'Lemon', l:'f_lemon', i: Citrus}, {v:'Berry', l:'f_berry', i: Cherry},
                  {v:'Orange', l:'f_orange', i: Sun}, {v:'Smoky', l:'f_smoky', i: Flame}, 
                  {v:'Earthy', l:'f_earthy', i: Sprout}, {v:'Peach', l:'f_peach', i: Apple},
                  {v:'Lychee', l:'f_lychee', i: CloudFog}
                ].map((f) => {
            ...
            <View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} className="bg-espresso-900/40 border border-espresso-700/50 rounded-3xl p-3">
                {[ {v:'Hand Drip', l:'e_handdrip', i: Droplet}, {v:'Espresso', l:'e_espresso', i: Coffee}, {v:'Capsule', l:'e_capsule', i: Pill}, {v:'Moka Pot', l:'e_mokapot', i: Flame}, {v:'French Press', l:'e_frenchpress', i: Beaker}].map((e) => {
        ...
        {step === 5 && recommendation && (
          <View className="space-y-6 pb-20">
             ...
             <View className="mt-6 mb-8 items-center w-full px-2">
                 <ScrollView className="max-h-[300px] w-full px-2" indicatorStyle="white">
                   <Markdown style={{
                     body: { color: '#d3cec7', fontSize: 13, lineHeight: 22 },
                     strong: { color: '#f59e0b', fontWeight: 'bold' },
                     bullet_list: { color: '#f59e0b' }
                   }}>
                     {aiExplanation}
                   </Markdown>
                 </ScrollView>
